...
def read_flow(filename):
    ...
    解决bug: # https://stackoverflow.com/questions/42483476/numpy-only-integer-scalar-arrays-can-be-converted-to-a-scalar-index-upgrading/44906423
    ...
    if 202021.25 != magic:
        ...
        # w = np.fromfile(f, np.int32, count=1) # numpy升级后有bug
        ...
        # data2d = np.resize(data2d, (h[0], w[0], 2)) # numpy升级后有bug
...
# ------------------------------------------------------------------------------------------------------
# by HaoZhang
def batch_flow_to_image(flow_batch, display=False):
    """
    Convert flow into middlebury color code image
    :param flow: optical flow map
    :return: optical flow image in middlebury color
    """
    iternum = flow_batch.shape[0] # batch_size
    img_batch = []
    ...
    for idx in range(iternum):
        ...
        img_batch.append(np.uint8(img))

    img = np.stack(img_batch)  # 合并为更高一维的tensor, 把所有的frame都合并到一个大 tensor中
    return img
...
def readFlow(fn):
    ...
    with open(fn, 'rb') as f:
        ...
        if 202021.25 != magic:
            ...
            # print("type of w,h{} , {}".format(type(w), type(h)))
            # print("w, h:", w,"---", h)  # node48上的 shanghaitech *.flo存在 w,h为 null 的 flo file
            w,h = w.item(), h.item() # numpy to python scalar
